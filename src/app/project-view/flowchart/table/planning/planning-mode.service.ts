...
export class PlanningModeService {
  ...
  readonly generatedSteps = computed<MissionStep[]>(() => {
    ...
    // Fallback: direct path optimization (original behavior)
  ...
  private generateStepsWithAStar(
    ...
    for (const wp of wps) {
      ...
      if (!result) {
        // A* failed - fallback to direct path for this waypoint
        ...
        const directSteps = this.generateStepsDirectly(
          ...
          0 // No lineup for fallback
        );
        allSteps.push(...directSteps);
        // Update pose assuming direct path succeeded
        currentPose = this.simulateFinalPose(currentPose, directSteps);
      } else {
        // Optimize path (merge consecutive same-type commands)
